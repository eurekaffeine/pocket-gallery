# 업데이트 로그
## Android

### V3.8

#### 3.8.2608162314

- 사용률 랭킹에서 이제 싱글 배틀 순위를 확인할 수 있습니다
- 기타 문제 수정

### V3.7

#### 3.7.2607041035

- 데미지 계산의 일부 문제 수정
- 챔피언스 게임 버전에 맞춰 포켓몬 기술 업데이트
- 새로운 메가 포켓몬의 특성 업데이트
- 기타 문제 수정

### V3.6

#### 3.6.2606181535

_(3.6.2605130901)_

- 데미지 계산기 및 사용률 랭킹 기능 추가
- 입수 방법 탭에 야생 출현 위치 추가
- 종족값 UI 표시 개선
- 진화 체인 UI 표시 개선
- 기타 문제 수정

### V3.5

#### 3.5.2604051948

- Pokémon Home의 공식 일러스트를 업데이트했습니다
- 포켓몬 기술 설명 오류를 수정했습니다
- 기타 문제를 수정했습니다

### V3.4

#### 3.4.2603091036

- 새로운 메가 포켓몬의 공식 일러스트를 업데이트했습니다
- 일부 언어의 번역을 개선했습니다
- 일부 문제를 수정했습니다

### V3.3

#### 3.3.2512281449

- 'LZA 인터랙티브 맵'을 추가했습니다
- 일부 문제를 수정했습니다

### V3.2

#### 3.2.2512142334

- 'Z-A 초차원 서지' DLC 데이터를 추가했습니다
- 일부 문제 수정

### V3.1

#### 3.1.2512071626

- 일부 문제 수정

#### 3.1.2511032310

- "LEGEND: Z-A"의 포켓몬 도감 및 기술 추가
- 일부 문제 수정

### V3.0

#### 3.0.2510061832

- 나옹(가라르의 모습)의 기술 누락 문제를 수정했습니다
- 모크나이퍼(히스이의 모습)의 잘못된 종족값 문제를 수정했습니다
- "팀 편집"에서 커스텀 포켓몬 이름을 지원합니다
- 기타 문제들을 수정했습니다

### V2.9
#### 2.9.2507061701
_(2.9.2503161542)_

- 사용자 인터페이스 최적화
- 기타 문제 수정

### V2.8
#### 2.8.2411161539 
_(2.8.2410191658)_

- 애플리케이션 시작 속도 최적화
- "팀 편집" 모드에서 "이색" 포켓몬 선택 지원
- "팀 편집" 모드에서 더 많은 "기술" 설정 지원
- 기타 문제 수정

### V2.7
#### 2.7.2408241416
- 일부 문제를 수정했습니다

#### 2.7.2408012240
- UX(사용자 경험) 개선
- 앱 크기 최적화
- 일부 문제를 수정했습니다

### V2.6
#### 2.6.2407230913
- 일부 문제 수정

#### 2.6.2406161146
_(2.6.2405191437)_
- 일부 문제 수정

### V2.5
#### 2.5.2405090925
- 이전 세대의 포켓몬 기술 지원
- 포켓몬 기술에 대한 기본 게임 버전 선택 지원
- 일부 문제 수정

### V2.4
#### 2.4.2404042223
- "팀 편집" 기능 지원
- 일부 문제 수정
### V2.3
#### 2.3.2403092340
- 일부 포켓몬의 포켓몬 도감 항목 업데이트
- FAQ 섹션 추가
- 일부 문제 수정

### V2.2
#### 2.2.2402161119
- GEN IX Ver2.0.1 에서 게임 데이터 업데이트
- GEN IX DLC vol.2에서 게임 데이터 업데이트
- 일부 문제 수정.

### V2.1
#### 2.1.2311052226
- 포켓몬 설명 표시。
- 일부 문제 수정。


### V2.0
#### 2.0.2310011943
- 사용자 인터페이스 및 경험 개선
- GEN IX DLC 1의 포켓몬 움직임 지원
- GEN IX DLC1의 지역 포켓몬도 지원
- 일부 문제 수정
- 포켓몬 이미지 자산 다운로드 지원


### V1.9
#### 1.9.2309272329
- DLC 1: 벽록의 가면 대한 추가 정보
- 일부 문제 수정

### V1.8
#### 1.8.2306041409
- GEN IX 포켓몬 홈 3.0 관련 정보가 업데이트되었습니다
- 포켓몬 홈의 공식 일러스트가 업데이트되었습니다
- 일부 문제가 수정되었습니다
### V1.7
#### 1.7.2305271730
- 일부 문제 및 버그 수정
#### 1.7.2305211308
- 「레츠고! 피카츄/이브이」, 「울트라썬/울트라문」 및 「Sun/Moon」의 포켓몬 설명 지원
- 기타 문제 수정
#### 1.7.2305160013
- 일부 문제 및 버그 수정
#### 1.7.2305071538
- 다국어 지원
- 일부 문제 및 버그 수정
### V1.6
#### 1.6.2304291857
- 기타 문제 수정
#### 1.6.2304072247
- "불폭풍 야수" GEN-IX 이동 추가
- 기타 문제 수정
#### 1.6.2304021810
- "대검 유령" GEN-IX 기술을 추가했습니다
- 기타 문제를 수정했습니다
#### 1.6.2303291135
_(1.6.2303261545)_
- "스나이퍼 샷 트리로드" GEN-IX 이동 추가
- 새로운 속성 컨트롤 테이블을 추가했습니다.
- 다른 언어로 된 포켓몬 번역 표시를 추가했습니다.
- 기타 문제 수정

### V1.5

#### 1.5.2303122105
- "Material You" 스타일의 앱 아이콘 추가
- 기타 문제 수정

#### 1.5.2303042104
- "잔물결 물"섹션의"철 점박이 나뭇잎"에 대한 정보 업데이트
- 기타 문제 수정
#### 1.5.2302022247
- 코가 시노비 패밀리의 GEN IX 동작을 업데이트했습니다
- 기타 문제 수정
#### 1.5.2301262229
_(1.5.2301241051)_
- 포켓몬 정보 페이지에 포켓몬 홈 아트워크 표시를 지원합니다
- 포켓몬 정보 페이지에서 성별 차이 표시를 지원합니다
- 기타 문제 수정
### V1.4
#### 1.4.2301151012
- GEN IX 공식 아트워크 업데이트
- GEN IX 속성 아이콘 업데이트
- 빠른 스와이프 목록 지원
#### 1.4.2301021806
- 새로운 공통 아이템으로 GEN IX 업데이트
- GEN IX 포켓몬 기술 업데이트
- GEN IX "눈" 날씨에 대한 설명이 업데이트되었습니다
#### 1.4.212311405
- "능력치 계산"및 "개별 값 계산"기능을 추가했습니다
- 기타 문제를 수정했습니다
### V1.3
#### 1.3.2212060006
- "불을 내뿜는 드래곤"계열과 "섬광 불꽃 에이스"계열의 이동 데이터를 업데이트했습니다
- "고대 제인 스네일"과 "고대 딩글"의 잘못된 속성을 수정했습니다
- 기타 문제를 수정했습니다.
#### 1.3.2211281254
- "파디야 지역"의 다른 포켓몬에 대한 설명을 업데이트했습니다
- "매듭초", "사쿠라", "껍질없는 바다토끼", "바다토끼야수", "사계절사슴", "새싹사슴", "화려한 핑크나비", "꽃꿀벌", "꽃잎티", "꽃여제지", "도레미야" 및 "돌고래맨"의 형태 변경 사항을 업데이트했습니다
- 기타 몇 가지 문제를 수정했습니다
### V1.2
#### 1.2.2211012343
- 몇 가지 문제를 수정했습니다
#### 1.2.2209121837
- 몇 가지 문제를 수정했습니다
#### 1.2.2209031605
- 일부 문제를 수정했습니다
#### 1.2.2208310047
- "포켓몬 전설의 알-제우스" 포켓몬을 추가합니다
#### 1.2.2208300859
- "매치메이킹 프로필"에서 "타격 블라인드"와 "합동 방어 블라인드"를 지원하도록 추가했습니다
- "매치메이킹 프로필"에서 "대미지" 계산을 지원하도록 추가했습니다
#### 1.2.2208281620 
- "매치메이킹 프로필"에 새로운 기능을 추가했습니다
- "매치메이킹 프로필"에 "속성 제한"을 지원하도록 추가했습니다
#### 1.2.2208211732
- "일반 정보" 섹션에 "지역 지도" 기능을 지원하도록 추가했습니다
### V1.1
#### 1.1.2207220018
- "자주 사용하는 정보" 기능을 추가했습니다
### V1.0
#### 1.0.2207062326
- 이 애플리케이션의 첫 번째 릴리스
### V0.2
#### 0.2.2206051518
- 앱의 초기 베타 오픈 테스팅

## iOS

### V1.4

#### 1.4.4

1. 레이아웃과 전반적인 사용 환경을 개선했습니다.
2. 새로운 메가진화 형태의 특성 정보를 추가했습니다.
3. 여러 문제를 수정했습니다.

#### 1.4.3

- 사용률 랭킹에서 이제 싱글 배틀 순위를 확인할 수 있습니다
- 기타 문제 수정

#### 1.4.2

- 데미지 계산의 일부 문제 수정
- 챔피언스 게임 버전에 맞춰 포켓몬 기술 업데이트
- 새로운 메가 포켓몬의 특성 업데이트
- 기타 문제 수정

#### 1.4.1

- 데미지 계산기 및 사용률 랭킹 기능 추가
- 입수 방법 탭에 야생 출현 위치 추가
- 종족값 UI 표시 개선
- 진화 체인 UI 표시 개선
- 기타 문제 수정

#### 1.4.0

- Pokémon Home의 공식 일러스트를 업데이트했습니다
- 포켓몬 기술 설명 오류를 수정했습니다
- 기타 문제를 수정했습니다

### V1.3

#### 1.3.9

- 새로운 메가 포켓몬의 공식 일러스트를 업데이트했습니다
- 일부 언어의 번역을 개선했습니다
- 일부 문제를 수정했습니다

#### 1.3.8

- 'LZA 인터랙티브 맵'을 추가했습니다
- 일부 문제를 수정했습니다

#### 1.3.7

- 'Z-A 초차원 서지' DLC 데이터를 추가했습니다
- 일부 문제 수정

#### 1.3.6

- 일부 문제 수정

#### 1.3.5

- 앱 크기 최적화
- 일부 문제 수정

#### 1.3.4

- "LEGEND: Z-A"의 포켓몬 도감 및 기술 추가
- 일부 문제 수정

#### 1.3.3

- 나옹 (가라르 폼)의 누락된 기술 수정
- 기타 일부 문제 수정

#### 1.3.2

- 일부 문제 수정

#### 1.3.1

- iOS 26 및 iPad OS 26에 대응

#### 1.3.0

- "배틀 정보" 페이지에 "팀 편집" 기능 추가
- 일부 문제 수정

### V1.2

#### 1.2.7

- 기타 문제 수정

#### 1.2.6

- 사용자 인터페이스 최적화
- 기타 문제 수정

#### 1.2.5

_(1.2.4) (1.2.3)_

- 일부 문제를 수정했습니다
#### 1.2.2
- "전투 데이터" 페이지에 "능력치 계산", "개인 값 계산" 기능을 추가했습니다
- 일부 문제를 수정했습니다

#### 1.2.1
- "배틀 정보" 페이지에 "공격의 맹점"과 "방어의 맹점"이 추가되었습니다
- 일부 문제를 수정했습니다

#### 1.2.0
- '전투 데이터' 페이지에 '타입 효율성' 및 '타입 데미지' 기능이 추가되었습니다
- 일부 문제를 수정했습니다

### V1.1
#### 1.1.2
- 새로운 위젯을 추가했습니다

#### 1.1.1
- 일부 문제 수정

#### 1.1.0
- 이전 세대의 포켓몬 기술 지원
- 포켓몬 기술에 대한 기본 게임 버전 선택 지원
- 일부 문제 수정

### V1.0
#### 1.0.4
- 일부 포켓몬의 포켓몬 도감 항목 업데이트
- FAQ 섹션 추가
- 일부 문제 수정
#### 1.0.3
- 일부 공식 아트워크 업데이트됨
- 일부 문제 수정됨
#### 1.0.2
- GEN IX Ver2.0.1 에서 게임 데이터 업데이트
- GEN IX DLC vol.2에서 게임 데이터 업데이트
- 일부 문제 수정
#### 1.0.1
- 충돌, 렉 및 기타 문제 수정
- iOS 17과의 호환성 문제 해결
#### 1.0.0
- 앱의 공식 출시


